...
class FinologRequisiteService(FinologAPIService):
    ...
    def get_requisites(self, **payload) -> List[Requisite]:
        ...
        self.validate_payload(
            payload=payload,
            types={"contractor_id": int, "ids": str, "is_bizzed": bool},
        )
    ...
    async def create_requisite(
        self, contractor_id: int, name: str, **payload
    ) -> Requisite:
        """
    ...
    async def update_requisite(self, id_: int, **payload) -> Requisite:
        """
        Creates and returns Requisite object.
        ...
        return Requisite(
            **(await self.request("PUT", f"{self.uri}/{str(id_)}", payload))
        )
    ...
    def _validate_update_or_create_payload(self, payload):
        ...
        if "bank_account" in payload:
            validate_string_length(
                "bank_account", payload["bank_account"], min_l=20, max_l=28
            )
        if "bank_bic" in payload:
            validate_string_length(
                "bank_bic", payload["bank_bic"], min_l=9, max_l=9
            )

    async def delete_requisite(self, id_: int) -> Requisite:
        self.validate_payload(payload={"id": id_}, types={"id": int})
